...
tangent[:,0] = tangent[:,1]
#print angle.max(),angle.min(),'*****',angle.shape
#angle	= np.tan(angle)
#print angle[:,0]
#exit()
...
#print grid_a[-1,:]
#exit()
ang_grid = (tan_max-tan_min)/1
rad_grid = data.size / ang_grid
print ang_grid,'grid number'
...
plt.xlabel('angle')
plt.ylabel('radius')
x_axis = np.linspace(np.tan(ang_min/180.*np.pi),np.tan(ang_max/180.*np.pi),data3.shape[1])
